(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  const getWorkerInstance = (url, options = {}, findWorker) => __awaiter(void 0, void 0, void 0, function* () {
      ...
      const [, mockRegistrations] = yield until$1(() => __awaiter(void 0, void 0, void 0, function* () {
...
Learn more about creating the Service Worker script: https://mswjs.io/docs/cli/init`);
  ...
  function getGraphQLQuery(request) {
      var _a;
      switch (request.method) {
          ...
          case 'POST': {
              ...
              return (_a = request.body) === null || _a === void 0 ? void 0 : _a.query;
          }
          default:
              return null;
      }
...
${handlers.map((handler) => `  • ${handler.getMetaInfo().header}`).join('\n')}`;
      ...
      return `Did you mean to request "${handlers[0].getMetaInfo().header}" instead?`;
  }
...
`,
  ...
  const createRequestListener = (context, options) => {
      return (event, message) => __awaiter(void 0, void 0, void 0, function* () {
          ...
          try {
              ...
              if (!handler) {
                  onUnhandledRequest(req, context.requestHandlers, options.onUnhandledRequest);
  ...
  function requestIntegrityCheck(context, serviceWorker) {
      return __awaiter(this, void 0, void 0, function* () {
          ...
          if (actualChecksum !== "dc3d39c97ba52ee7fff0d667f7bc098c") {
              throw new Error(`Currently active Service Worker (${actualChecksum}) is behind the latest published one (${"dc3d39c97ba52ee7fff0d667f7bc098c"}).`);
          }
  ...
  const createStart = (context) => {
      ...
      return function start(options) {
          ...
          const startWorkerInstance = () => __awaiter(this, void 0, void 0, function* () {
              ...
              const [, instance] = yield until$1(() => getWorkerInstance(resolvedOptions.serviceWorker.url, resolvedOptions.serviceWorker.options, resolvedOptions.findWorker));
              if (!instance) {
                  return null;
              }
...
If this message still persists after updating, please report an issue: https://github.com/open-draft/msw/issues\
      `);
              ...
              const [activationError] = yield until$1(() => activateMocking(context, options));
              if (activationError) {
...
`);
  ...
  function parseQuery$1(query, definitionOperation = 'query') {
      var _a;
      const ast = parse$1(query);
      const operationDef = ast.definitions.find((def) => {
          return (def.kind === 'OperationDefinition' &&
  ...
  function buildMultipartVariables(variables, map, files) {
              ...
              switch (req.method) {
                  case 'GET': {
                      ...
                      const variables = variablesString
                          ...
                          : {};
                      const { operationType, operationName } = parseQuery$1(query, expectedOperationType);
                      return {
                          operationType,
                          operationName,
                          variables,
                  ...
                  case 'POST': {
                      ...
                      if ((_b = req.body) === null || _b === void 0 ? void 0 : _b.operations) {
                          const _c = req.body, { operations, map } = _c, others = __rest(_c, ["operations", "map"]);
                          const parsedOperations = jsonParse(operations) || {};
                          if (!parsedOperations.query) {
                              return null;
                          }
                          const parsedMap = jsonParse(map) || {};
                          const { operationType, operationName } = parseQuery$1(parsedOperations.query, expectedOperationType);
                          const files = others;
                          let variables;
                          try {
                              variables = parsedOperations.variables
                                  ? buildMultipartVariables(parsedOperations.variables, parsedMap, files)
                                  : {};
                          }
                          catch (error) {
                              return null;
